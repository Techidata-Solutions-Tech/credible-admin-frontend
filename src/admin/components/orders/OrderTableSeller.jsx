...
const OrderTable = () => {
  ...
  const orderData = [
    { date: '2024-01-10', id: '001', customerName: 'John Doe', branchId: 'BR001', items: 5, qty: 10, orderValue: 250.00 },
    { date: '2024-02-15', id: '002', customerName: 'Jane Smith', branchId: 'BR002', items: 3, qty: 7, orderValue: 150.50 },
    { date: '2024-03-20', id: '003', customerName: 'Alice Johnson', branchId: 'BR003', items: 8, qty: 20, orderValue: 400.75 },
  ...
  const handleConfirmDelete = async () => {
    setLoading(true);
    try {
      await new Promise((resolve) => setTimeout(() => resolve('Order deleted'), 2000));
      setLoading(false);
    ...
    <>
      ...
      <dialog id="product_detail" className="modal">
        <form method="dialog" className="modal-box">
          <button type="button" className="absolute text-3xl top-2 right-2 text-gray-500 hover:text-gray-800" onClick={() => document.getElementById('product_detail').close()}>&times;</button>
      ...
      <dialog id="my_modal_edit" className="modal">
        <form method="dialog" className="modal-box">
          <button type="button" className="absolute text-3xl top-2 right-2 text-gray-500 hover:text-gray-800" onClick={() => document.getElementById('my_modal_edit').close()}>&times;</button>
          <h2 className="text-lg font-semibold uppercase">Edit Order</h2>
          <input type="text" value={modalData?.customerName || ''} onChange={(e) => setModalData({ ...modalData, customerName: e.target.value })} className="input input-bordered w-full mt-2" placeholder="Customer Name" />
          <input type="text" value={modalData?.branchId || ''} onChange={(e) => setModalData({ ...modalData, branchId: e.target.value })} className="input input-bordered w-full mt-2" placeholder="Branch ID" />
          <input type="number" value={modalData?.items || ''} onChange={(e) => setModalData({ ...modalData, items: e.target.value })} className="input input-bordered w-full mt-2" placeholder="Items" />
          <input type="number" value={modalData?.qty || ''} onChange={(e) => setModalData({ ...modalData, qty: e.target.value })} className="input input-bordered w-full mt-2" placeholder="Quantity" />
          <input type="number" value={modalData?.orderValue || ''} onChange={(e) => setModalData({ ...modalData, orderValue: e.target.value })} className="input input-bordered w-full mt-2" placeholder="Order Value" />
      ...
      <dialog id="my_modal_delete" className="modal">
        <form method="dialog" className="modal-box">
          <button type="button" className="absolute text-3xl top-2 right-2 text-gray-500 hover:text-gray-800" onClick={() => document.getElementById('my_modal_delete').close()}>&times;</button>
          <h2 className="text-lg font-semibold">Are you sure you want to delete this order?</h2>
          <button className="btn btn-danger mt-4" onClick={handleConfirmDelete} disabled={loading}>
