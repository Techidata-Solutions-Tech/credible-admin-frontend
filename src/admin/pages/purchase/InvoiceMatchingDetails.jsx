...
function InvoiceMatchingDetails() {
  ...
  const totals = {
    invQty: productData.reduce(
      (sum, item) => sum + parseInt(item.invQty || 0),
      0
    ),
    deductionQty: productData.reduce(
      (sum, item) => sum + parseInt(item.deductionQty || 0),
      0
    ),
    deductionAmt: productData.reduce(
      (sum, item) => sum + parseInt(item.deductionAmt || 0),
      0
    ),
  ...
  const breadcrumbItems = [
    ...
    { label: "Invoice", href: "#" },
    { label: "Invoice Details", href: "/admin/approval/product/supplier" },
  ];

  return (
    <div className="p-4">
      <Breadcrumbs pageTitle="Invoice Details" items={breadcrumbItems} />

    <div className="flex justify-between my-5">
    <div className="flex  flex-col gap-3">
    <div className="flex justify-between gap-4 items-center"><span>
          Warehouse Location:{" "}</span>
          <span className="inline-block p-2 bg-gray-100 border border-black">
            {" "}
            Banglore
          </span>
        </div>
        <div className="flex justify-between gap-4 items-center"><span> Warehouse Id:{" "}</span>
          <span className="inline-block p-2 bg-gray-100 border border-black">
            {" "}
            20
          </span>
      ...
      <div className="flex flex-col gap-3">
      <div className="flex justify-between gap-4 items-center"><span>Supplier Name: </span>
      <span className="inline-block p-2 bg-gray-100 border border-black">
            {" "}
            Banglore
          </span>
      ...
      <div className="flex justify-between gap-4 items-center"><span>Supplier Id{" "}</span>
          <span className="inline-block p-2 bg-gray-100 border border-black">
            {" "}
            20
          </span>
        ...
        <table className="product-table">
          <thead>
            <tr className="group-header bg-sky-500 text-white text-sm text-center">
              <th
                rowSpan="2"
                className="text-center border px-2 py-1 border-black min-w-[110px]"
              >
                Date
              </th>
              ...
              <th
                colSpan="5"
                className="text-center border px-2 py-1 border-black"
              >
                Product
              </th>
              <th
            ...
            <tr className="sub-header bg-sky-300 text-black text-sm text-center">
              <th className="text-center border px-2 py-1 border-black">
                Product Name
              </th>
              <th className="text-center border px-2 py-1 border-black">
                Company/Brand
          ...
          <tbody>
            {productData.map((product, index) => (
              <tr key={index}>
                ...
                <td className="border px-2 py-1 border-black">
                  {product.deductionQty}
                </td>
                <td className="border px-2 py-1 border-black">
                  {product.deductionAmt}
                </td>
                <td className="border px-2 py-1 border-black">
                  {product.netQty}
                </td>
            ...
            <tr className="total-row">
              <td className="border px-2 py-1 border-black" colSpan="7">
                Total
              </td>
              <td className="border px-2 py-1 border-black">{totals.invQty}</td>
              <td className="border px-2 py-1 border-black">
                {productData.reduce(
                  (sum, item) => sum + parseInt(item.invValue || 0),
                  0
                )}
              </td>
              <td colSpan="3">-</td>
